...
socket.on("take all jobs", (data) => {
    fillfull(data).then(() => {
        if (!connected) {
            setTimeout(() => {
                loader.html(
                    `<i class="bi bi-heart text-pink" style="font-size: 64px"></i>`
                );
...
socket.on("redis connected", () => {
    ...
    $(this)
        ...
        .queue(() => {
            loader.html(
                `<i class="bi bi-chevron-bar-contract" style="font-size: 64px"></i>`
            );
            loadingHeader.text("Getting Data");
            loadingDescription.text("We getting and gathering data.");

            if (socketClientId) {
                loadingDescription.append(
                    `<span class="d-block mt-1 text-primary">Your socket id: ${socket.id}</span>`
                );
            }
...
const connect = (retry = false, round = 0) => {
    ...
    if (retry) {
        loadingHeader.html(
            `<span class='text text-danger'>Retry connecting...<span>`
        );
        loadingDescription.html(
            "Connection failed, we will keep retrying, please make sure that your redis server is started."
        );

        if (round && round > 0) {
            loadingDescription.append(
                `<strong class='d-block mt-3 text text-info'>Round: ${round}</span>`
            );
        }
    }
    ...
    return new Promise((resolve, reject) => {
            ...
            if (key === "completed") {
                ...
                if (x.length) {
                    x.forEach((job) => {
                        ...
                        msg.addClass("mt-3");
                        msg.addClass("p-3");
                        msg.append(`<strong>${job.data.SbjectName}</strong>`);
                        const resources = (xjob) => {
                            if (xjob.data.resources && xjob.data.resources.length) {
                                return xjob.data.resources.map(r => {
                                    return `<li><a href="${r.link}">${r.title}</a></li>`
                                })
                            }
                        }
                        msg.append(`<div>
        ...
        for (let key in jobs) {
            ...
            Array.from(jobs[key]).forEach((job, jobKey) => {
                ...
                console.log(job.data.SbjectName, job.data, job);
                jobComponent.html(`
                ...
                socket.on(`job-${job.id}-progress`, (data) => {
                    $(`#job-${job.id}-progress .job-progress`).css(
                        "width",
                        `${data}%`
                    );
                });
